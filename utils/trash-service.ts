...
/**
 * Opens the database once and shares the in-flight promise with concurrent callers.
 * This prevents multiple openDatabaseAsync calls during app startup, which can
 * produce native SQLite errors on Android.
 */
...
/**
 * Initializes the database exactly once at a time. The migration also upgrades
 * the Phase 2A trashed_media schema so expires_at can legitimately be NULL for
 * the "Never" retention option.
 */
export async function initialize(): Promise<void> {
  ...
  initializationPromise = (async () => {
    ...
    // Phase 2A created expires_at as NOT NULL. Rebuild the table once so the
    // Phase 2D "Never" option can store NULL without losing existing rows.
    ...
    if (expiresColumn?.notnull === 1) {
      await db.withTransactionAsync(async () => {
        await db.execAsync(`
          ...
          SELECT id, file_name, file_type, file_size, date_created, source, uri, duration, thumbnail_color, deleted_at, expires_at
          FROM trashed_media;
